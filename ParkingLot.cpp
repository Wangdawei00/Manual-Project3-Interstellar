...
void ParkingLot::drawingInitialize() {
    ...
    if (flagMovingCar){
        if (flag1){
            ...
            if (compare(momentAnchor.getY(),corner1.getY())&&compare(momentAnchor.getX(),corner1.getX())){ //if vehicle moves to the first corner
                flag1=false;
                cout<<"hu"<<endl;
            }
            ...
            if (compare(momentAnchor.getY(),corner2.getY())&&compare(momentAnchor.getX(),corner2.getX())) { //if vehicle moves to the second corner
                flag3=false;
            }
        }else {
            ...
            if (index == 15) { //if vehicle has finished rotating
                ...
                flag2=true;
                flag3=true;
                cout<<"slot=["<<ParkingLot::find_empty_slot()->getI()<<"]["<<ParkingLot::find_empty_slot()->getJ()<<"]"<<endl;
                slots[ParkingLot::find_empty_slot()->getI()][ParkingLot::find_empty_slot()->getJ()].pushGroup(all.back());
                index=0;
            }
        }
    }
